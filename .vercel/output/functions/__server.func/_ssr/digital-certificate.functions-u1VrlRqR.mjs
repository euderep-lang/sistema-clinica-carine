...
import { f as forge } from "../_libs/node-forge.mjs";
import { randomBytes, createCipheriv, createHash, createDecipheriv, scryptSync } from "node:crypto";
import { p as plainAddPlaceholderExports } from "../_libs/signpdf__placeholder-plain.mjs";
import { s as signpdfExports } from "../_libs/signpdf__signpdf.mjs";
import { P as P12SignerExports } from "../_libs/signpdf__signer-p12.mjs";
import process$1 from "node:process";
import { I as ITI_VALIDATOR_URL, S as SIGNATURE_STAMP_GAP_MM } from "./prescription-pdf-BEoUnrBo.mjs";
...
import "../_libs/signpdf__placeholder-pdfkit010.mjs";
import "../_libs/signpdf__utils.mjs";
...
async function signPdfBuffer(pdfBuffer, opts) {
  const withPlaceholder = plainAddPlaceholderExports.plainAddPlaceholder({
    pdfBuffer,
  ...
  const signer = new P12SignerExports.P12Signer(opts.pfxBuffer, { passphrase: opts.password });
  const signPdf = new signpdfExports.SignPdf();
...
const safeIdRedirectUriSchema = stringType().url().refine((u) => u.endsWith("/professional/safeid/callback"), {
