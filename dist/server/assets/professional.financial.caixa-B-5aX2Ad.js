...
import { t as todayISO, l as loadProfessionalExpenses } from "./expenses-BvcenOS4.js";
import { u as useAuth, L as Label, I as Input, X as fmt, W as fmtDate, C as Card, a as CardHeader, b as CardTitle, c as CardContent, m as Badge, aj as PAYMENT_LABEL, E as Table, F as TableHeader, G as TableRow, H as TableHead, J as TableBody, M as TableCell } from "./router-wbAJq94_.js";
...
function ProfessionalCaixaPage() {
  const {
    profile
  } = useAuth();
  ...
  const load = useCallback(async () => {
    ...
    const [{
      data: payData
    }, expenseData] = await Promise.all([supabase.from("bill_payments").select("id, amount, net_amount, fee_amount, payment_method, paid_date, notes, patients(full_name), bills_receivable(description)").eq("professional_id", profile.id).eq("status", "active").eq("paid_date", date).order("created_at", {
      ascending: false
  ...
  const filteredPayments = useMemo(() => {
    ...
    return payments.filter((p) => (p.patients?.full_name?.toLowerCase().includes(q) ?? false) || (p.bills_receivable?.description?.toLowerCase().includes(q) ?? false) || (p.notes?.toLowerCase().includes(q) ?? false));
  }, [payments, search]);
  ...
  return /* @__PURE__ */ jsx(FinancialShell, { children: /* @__PURE__ */ jsxs("div", { className: "space-y-6", children: [
    /* @__PURE__ */ jsxs("div", { className: "flex flex-wrap items-end gap-4", children: [
      ...
      /* @__PURE__ */ jsx(Input, { placeholder: "Buscar movimentação…", value: search, onChange: (e) => setSearch(e.target.value), className: "w-64" })
    ] }),
...
export {
  ProfessionalCaixaPage as component
};
