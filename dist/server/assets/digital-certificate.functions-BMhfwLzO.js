import { c as createServerRpc } from "./createServerRpc-BoWquCkD.js";
...
import { I as ITI_VALIDATOR_URL, S as SIGNATURE_STAMP_GAP_MM } from "./prescription-pdf-COE8s84u.js";
...
const initiateSafeIdSignatureAuth = createServerFn({
  ...
  redirectUri: safeIdRedirectUriSchema
})).handler(initiateSafeIdSignatureAuth_createServerFn_handler, async ({
  data,
  context
}) => {
  const profile = await requireProfessional(context.supabase, context.userId);
...
const getSafeIdSignatureAuthStatus = createServerFn({
  method: "GET"
}).middleware([requireSupabaseAuth]).handler(getSafeIdSignatureAuthStatus_createServerFn_handler, async ({
  context
}) => {
  const profile = await requireProfessional(context.supabase, context.userId);
  const {
