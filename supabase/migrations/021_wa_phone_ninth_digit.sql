-- Corrige normalização de celular (8 dígitos começando em 6–9) e funde duplicatas.
...
BEGIN
  FOR rec IN
    SELECT tenant_id, public.wa_phone_tail11(public.wa_normalize_br_phone(contact_phone)) AS tail
    FROM public.wa_conversations
    GROUP BY tenant_id, public.wa_phone_tail11(public.wa_normalize_br_phone(contact_phone))
    HAVING count(*) > 1
  LOOP
    SELECT c.id INTO keeper_id
    FROM public.wa_conversations c
    WHERE c.tenant_id = rec.tenant_id
      AND public.wa_phone_tail11(public.wa_normalize_br_phone(c.contact_phone)) = rec.tail
...
WHERE c.patient_id = p.id
  ...
  AND (c.contact_name IS NULL OR c.contact_name ~ '^[0-9]+$' OR c.contact_name IS DISTINCT FROM p.full_name);
