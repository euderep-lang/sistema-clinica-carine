-- Quem assume a taxa no recebimento: empresa (padrão) ou cliente.
-- Cliente: não calcula taxa nem gera despesa; valor integral no financeiro.

ALTER TABLE public.bill_payments
  ADD COLUMN IF NOT EXISTS fee_bearer text NOT NULL DEFAULT 'company'
    CHECK (fee_bearer IN ('company', 'client'));

DROP FUNCTION IF EXISTS public.receive_bill_payment(uuid, numeric, text, date, text, numeric, integer);
...
DECLARE
  ...
  v_fee_bearer text := COALESCE(NULLIF(trim(p_fee_bearer), ''), 'company');
BEGIN
  IF v_fee_bearer NOT IN ('company', 'client') THEN
    RAISE EXCEPTION 'Responsável pela taxa inválido';
  END IF;
  ...
  IF p_amount > 0 THEN
    ...
    IF v_fee_bearer = 'client' THEN
      v_fee := 0;
      v_net := round(p_amount, 2);
      v_settlement_days := 0;
    END IF;
    ...
    INSERT INTO public.bill_payments (
      ...
      installments, fee_expense_id, fee_bearer
    ) VALUES (
      v_bill.tenant_id,
      p_bill_id,
      v_bill.patient_id,
  ...
  RETURN jsonb_build_object(
    ...
    'fee_bearer', v_fee_bearer
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.receive_bill_payment(uuid, numeric, text, date, text, numeric, integer, text) TO authenticated;
