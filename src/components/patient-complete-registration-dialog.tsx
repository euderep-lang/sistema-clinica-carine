...
export function PatientCompleteRegistrationDialog({
  ...
  const validate = () => {
    ...
    if (!form.gender.trim()) e.gender = "Obrigatório";
    if (!form.birth_date.trim()) e.birth_date = "Obrigatório";
    if ((form.phone ?? "").replace(/\D/g, "").length < 8) e.phone = "Informe o WhatsApp";
    if ((form.address_zip ?? "").replace(/\D/g, "").length !== 8) e.address_zip = "CEP inválido";
    if (!form.address_street.trim()) e.address_street = "Obrigatório";
    if (!form.address_number.trim()) e.address_number = "Obrigatório";
    if (!form.address_neighborhood.trim()) e.address_neighborhood = "Obrigatório";
    if (!form.address_city.trim()) e.address_city = "Obrigatório";
    if (form.address_state.trim().length < 2) e.address_state = "Obrigatório";
    setErrors(e);
  ...
  const onSubmit = async (ev: React.FormEvent) => {
    ...
    onCompleted(data as CompleteRegistrationPatient);
  };
