...
export function CrmPwaInstallBanner() {
  ...
  return (
    <div
      className="mx-2 mb-2 shrink-0 rounded-lg border border-[#075E54]/20 bg-[#dcf8c6] px-3 py-2.5 text-sm text-[#111b21] shadow-sm dark:bg-emerald-950/40 dark:text-emerald-50"
      style={{
    ...
    >
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {iosHint ? (
          <Button size="sm" variant="secondary" className="h-8 min-w-0 flex-1 text-xs" onClick={dismiss}>
            <Share className="mr-1.5 size-3.5 shrink-0" />
            Entendi
          </Button>
        ) : (
          <Button
            size="sm"
            className="h-8 min-w-0 flex-1 bg-[#075E54] text-xs text-white hover:bg-[#064e46]"
