...
import { buildMealPlanSystemPrompt } from "@/lib/ai-prompts.server";
import { getSupabaseServerEnv } from "@/lib/supabase-env.server";

/**
 * Endpoint de streaming (SSE) do chat do Plano Terapêutico.
...
export async function handleMealPlanChat(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error("[meal-plan-chat] context build failed", e);
  }

  const systemPrompt = buildMealPlanSystemPrompt(prof, systemContext);
